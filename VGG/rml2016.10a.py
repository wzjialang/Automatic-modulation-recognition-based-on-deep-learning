...
model = vggnet.VGGLikeModel(weights=None,input_shape=[128,2])
...
filepath = 'weights/VGGlike_dr0.5.h5'
history = model.fit(X_train,
    ...
    callbacks = [reduce_lr,
                ...
                #keras.callbacks.TensorBoard(log_dir='./logs/',write_graph=False,update_freq='epoch')
                ]
                    )
# history = model.fit(X_train,
#     Y_train,
...
def predict(model,filename="data/RML2016.10a_dict.pkl"):
    ...
    print(acc)
    fd = open('predictresult/VGGlike_dr0.5.dat','wb')
    ...
    plt.xlabel("Signal to Noise Ratio")
    plt.ylabel("Classification Accuracy")
    plt.title("VGGlike Classification Accuracy on RadioML 2016.10 Alpha")
    plt.tight_layout()
