...
#model = cldnn.CLDNNLikeModel(None,input_shape=[128,2],classes=11)
model = resnet.ResNetLikeModel(None,input_shape=[128,2])
model.compile(loss='categorical_crossentropy', metrics=['accuracy'], optimizer='adam')
#plot_model(model, to_file='model_ResNet.png',show_shapes=True) # print model
model.summary()
...
def predict(model,filename="data/RML2016.10a_dict.pkl"):
    ...
    fd = open('predictresult/acc_for_mod_on_res.dat', 'wb')
    pickle.dump(('128','res', acc_mod_snr), fd)
    fd.close()

    # Save results to a pickle file for plotting later
    print(acc)
    fd = open('predictresult/ResNet.dat','wb')
    ...
    plt.xlabel("Signal to Noise Ratio")
    plt.ylabel("Classification Accuracy")
    plt.title("ResNet Classification Accuracy on RadioML 2016.10 Alpha")
    plt.tight_layout()
